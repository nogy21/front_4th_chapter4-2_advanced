...
const SearchDialog = memo(
  ({ searchInfo, onClose }: Props) => {
    ...
    // 검색 옵션이 변경될 때만 필터링 수행
    const performFiltering = useCallback(() => {
      console.log('필터링 수행', performance.now());
    ...
    }, [searchOptions, lectures, performFiltering]);

    // 현재 페이지에 표시할 강의 목록
    const visibleLectures = useMemo(() => {
      if (!filteredResults) return [];
      return filteredResults.filteredLectures.slice(0, page * PAGE_SIZE);
    ...
    useEffect(() => {
      ...
      if (!$loader || !$loaderWrapper || !filteredResults) {
        return;
      }
      ...
      const observer = new IntersectionObserver(
        (entries) => {
          if (entries[0].isIntersecting) {
            setPage((prevPage) => Math.min(filteredResults.lastPage, prevPage + 1));
          }
        },
        ...
        <ModalContent maxW="90vw" w="1000px">
          ...
          <ModalBody>
            <VStack spacing={4} align="stretch">
                ...
                <FormControl>
                  ...
                  >
                    ...
                    <Stack
                      ...
                      p={2}
                    >
                      {filteredResults.allMajors.map((major) => (
                        <Box key={major}>
              ...
              <Text align="right">검색결과: {filteredResults.filteredLectures.length}개</Text>
              <Box>
                <Table>
                  <Thead>
                    <Tr>
  ...
  (prevProps, nextProps) => {
    return (
      prevProps.searchInfo?.tableId === nextProps.searchInfo?.tableId &&
